...
/*
						if (buffer_img != nullptr) {
						    // Очищаем вектор, на который указывает buffer_img
						    buffer_img->clear();

						    // Удаляем указатель buffer_img, если это необходимо
						    delete buffer_img;

						    // Теперь можно установить указатель buffer_img в nullptr, чтобы избежать ошибок доступа к памяти
						    buffer_img = nullptr;
						}
*/
...
memset(&output_buffer,'\0', sizeof(output_buffer));
						...
						/*
						for(int z = 0;z<sizeof(get_buffer);z++)
						        	{
						        		printf("%02X ", get_buffer[z]);
						        	}
						cout<<endl;
						...
						int optval=1;
						while (optval!=0){
							optval=recv(sock, buffer, sizeof(buffer), MSG_DONTWAIT);
							if(optval==-1){
								cout<<"ERROR"<<endl;
								pause();
							}
...
/*
						 int optval = 0;
						 int result = setsockopt(sock, SOL_SOCKET, SO_RCVBUF, &optval, sizeof(optval));
						       if (result == 0) {
						           printf("Буфер очищен!\n");
						           cout <<endl;
						       } else {
						           printf("Ошибка при выполнении setsockopt\n");
						           cout <<endl;

						       }

						      cout<<"BUFFER SOCKET:"<<endl;

						      // Чтение данных из сокета в буфер
						      ssize_t   bytesRead = recv(sock, buffer, sizeof(buffer), 0);

						         if (bytesRead > 0) {
cout<<"Size Bytes Read:"<<bytesRead<<endl;
						         } else if (bytesRead == 0) {
						             std::cout << "Соединение закрыто клиентом." << std::endl;
						         } else {
						             std::cout << "Ошибка при чтении данных из сокета." << std::endl;
						         }
						         */
						...
						switch (rezhim){
							...
							case 0x02:{
								...
								if (photo_after_alarm_done == 1){
									translate_buffer[23] = 0x00;//photo_after_alarm.size() >> 16;;
									translate_buffer[24] = 0x00;//(photo_after_alarm.size() >> 8) & 0x00FF;
									translate_buffer[25] = 0x00;///photo_after_alarm.size() & 0x0000FF;
								}
...
cout << "NUMBER OUT_DATA:="<<number_out_data<<endl;
    // Проверка на переполнение
    if (number_out_data > 0xFFFF) {//0x0FFF
        printf("Переполнение\n");
        exit(EXIT_FAILURE);
        break;
    }
    ...
    switch (number_stop_kadr) {
        case 4: {
        	...
        	for(int z = 0;z<34;z++)
        	{
        		printf("%02X ", photo_alarm[start_kadr+z]);
        	}
        	printf("\n");
            memcpy(translate_buffer + 16, &photo_alarm[start_kadr], size_kadr);
            /*printf("translate_buf: \n");
            for(int z = 0;z<34;z++)
            {
            	printf("%2X ", translate_buffer[start_kadr+16+z]);
            }
            printf("\n");*/
